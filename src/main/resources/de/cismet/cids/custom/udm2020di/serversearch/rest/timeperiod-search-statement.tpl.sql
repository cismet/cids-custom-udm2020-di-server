SELECT ocid,
  ...
  lightweight_json
    ...
    FROM
      ...
      AND (
        CSX_OBJECT_TAG_TIMESTAMP.MIN_DATE BETWEEN (TO_DATE('%MIN_DATE%', 'YYYY-MM-DD')-1) AND (TO_DATE('%MAX_DATE%', 'YYYY-MM-DD')+1)
        OR CSX_OBJECT_TAG_TIMESTAMP.MAX_DATE BETWEEN (TO_DATE('%MIN_DATE%', 'YYYY-MM-DD')-1) AND (TO_DATE('%MAX_DATE%', 'YYYY-MM-DD')+1)
        OR ( CSX_OBJECT_TAG_TIMESTAMP.MIN_DATE < (TO_DATE('%MIN_DATE%', 'YYYY-MM-DD')-1) AND (TO_DATE('%MAX_DATE%', 'YYYY-MM-DD')+1) < CSX_OBJECT_TAG_TIMESTAMP.MAX_DATE )
        )
